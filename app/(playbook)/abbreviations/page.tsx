...
const abbreviations: AbbreviationEntry[] = [
  ...
  },
  {
    ...
    example: 'If you trained a model on 3 years of your brand\'s copy and style guide, it would write in your exact voice without needing a lengthy style brief in every prompt. That customised model is a Fine-tuned AI Layer, with fewer prompt iterations and consistently on-brand output.',
...

  {
    abbr: 'T2I',
    full: 'Text-to-Image',
    meaning: 'Generating images from text prompts: write a description and get an image. Enables rapid prototyping for visual ideas.',
    ...
    <div style={{ borderBottom: isLast ? 'none' : '1px solid rgba(255,255,255,0.05)' }}>
      ...
      >
        <div className="px-4 pb-4 space-y-3" style={{ paddingTop: '2px' }}>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 19, color: 'rgba(255,255,255,0.6)', lineHeight: 1.6 }}>{item.meaning}</p>
          <div
            style={{ background: 'rgba(155,63,255,0.07)', border: '1px solid rgba(155,63,255,0.15)', borderRadius: 10, padding: '12px 14px' }}
          >
            <div style={{ color: '#C27FFF', fontSize: 19, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', fontFamily: 'var(--font-body)', marginBottom: 6 }}>
              How it&apos;s used
            </div>
            <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 19, fontFamily: 'var(--font-body)', lineHeight: 1.6 }}>{item.example}</p>
          </div>
          {item.links.length > 0 && (
        ...
        <div className="mb-6">
          ...
          >
            ...
            <input
              ...
              style={{ flex: 1, background: 'transparent', outline: 'none', fontSize: 19, fontFamily: 'var(--font-body)', color: 'rgba(255,255,255,0.88)', border: 'none' }}
            />
            {query && (
              <button onClick={() => setQuery('')} style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 4, color: 'rgba(255,255,255,0.45)', fontSize: 19, padding: '2px 8px', cursor: 'pointer' }}>
                Clear
              </button>
            )}
          </div>
            ...
            {filteredByGroup.map(group => (
              <div key={group.id}>
                <div style={{ background: group.bg, borderRadius: 8, padding: '6px 12px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ color: group.color, fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{group.label}</span>
