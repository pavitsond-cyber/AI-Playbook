          ...
          {navItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} style={{
                ...
                fontFamily: 'var(--font-body)', fontSize: 19, fontWeight: 400,
      ...
      {mobileOpen && (
        <div className="sm:hidden" style={{
          ...
          <nav style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Link href="/" onClick={() => setMobileOpen(false)} style={{ padding: '10px 12px', borderRadius: 8, fontFamily: 'var(--font-body)', fontSize: 19, color: pathname === '/' ? '#ffffff' : 'rgba(255,255,255,0.55)', background: pathname === '/' ? 'rgba(255,255,255,0.08)' : 'transparent', textDecoration: 'none' }}>Home</Link>
            {navItems.map(item => (
              <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} style={{ padding: '10px 12px', borderRadius: 8, fontFamily: 'var(--font-body)', fontSize: 19, color: isActive(item.href) ? '#ffffff' : 'rgba(255,255,255,0.55)', background: isActive(item.href) ? 'rgba(255,255,255,0.08)' : 'transparent', textDecoration: 'none' }}>
                {item.label}
              </Link>
            ))}
          </nav>
